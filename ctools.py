...
# --- NEW ---
# Add the YouTube API Key from your .env file
YOUTUBE_API_KEY = os.environ.get("YOUTUBE_API_KEY")
NOTION_API_KEY = os.environ.get("NOTION_API_KEY")
NOTION_DATABASE_ID = os.environ.get("NOTION_DATABASE_ID")
# --- END NEW ---

# ---- Helpers for token file handling (for OAuth) ----
...
# ---- Google service builder (for OAuth tools) ----
...
# --- NEW ---
# ---- YouTube service builder (uses a global API key) ----
...
def add_note_to_notion(content: str) -> str:
    ...
    # --- START OF FIX ---
        ...
        # This new logic safely handles all error types
        error_message = str(e) # Default to the basic error
        if e.response:
            try:
                # Try to get the JSON error message from Notion
...
def get_notes_from_notion(limit: int = 5) -> str:
    ...
    # We will sort by "created_time" to get the newest notes first
    # and limit the results using the "page_size" parameter.
    payload = {
        "page_size": limit,
        "sorts": [
            {
                "property": "Created time", # This is a default property
    ...
    try:
        ...
        note_list = []
        for page in results:
            # This complex-looking line just extracts the text from the 'Name' property
...
# ---- Lightweight FastAPI OAuth app (runs on port 8081) ----
...
def run_auth_app():
    # Run uvicorn for auth_app on port 8081
...
if __name__ == "__main__":
    ...
    # Start FastMCP tools server (blocking call) - this exposes the tools on /mcp
    try:
